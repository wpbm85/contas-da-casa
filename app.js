...
const ICONS={"FINANCIAMENTO": '<i class="fa-solid fa-landmark"></i>', "CONDOMINIO": '<i class="fa-solid fa-building"></i>', "IPTU / VAGA": '<i class="fa-solid fa-car-rear"></i>', "INTERNET": '<i class="fa-solid fa-wifi"></i>', "COMGAS": '<i class="fa-solid fa-fire"></i>', "ENEL": '<i class="fa-solid fa-bolt"></i>', "FAXINA": '<i class="fa-solid fa-broom"></i>', "NETFLIX": '<i class="fa-solid fa-tv"></i>', "ML/DISNEY": '<i class="fa-solid fa-film"></i>', "AMAZON": '<i class="fa-solid fa-box"></i>', "SEGURO BOLSA": '<i class="fa-solid fa-bag-shopping"></i>', "SEGURO APTO": '<i class="fa-solid fa-house-chimney"></i>', "SEGURO CARRO": '<i class="fa-solid fa-car-burst"></i>', "IPVA": '<i class="fa-solid fa-receipt"></i>', "ESCOLA": '<i class="fa-solid fa-graduation-cap"></i>', "CONVENIO": '<i class="fa-solid fa-stethoscope"></i>', "PEDIATRA": '<i class="fa-solid fa-user-doctor"></i>', "DENTISTA": '<i class="fa-solid fa-tooth"></i>', "ESPORTE": '<i class="fa-solid fa-person-running"></i>', "FARMA/SAÚDE": '<i class="fa-solid fa-pills"></i>', "ROUPAS": '<i class="fa-solid fa-shirt"></i>', "PRESENTES": '<i class="fa-solid fa-gift"></i>', "FESTA": '<i class="fa-solid fa-cake-candles"></i>', "HSP": '<i class="fa-solid fa-hospital"></i>', "CONV ODONTO": '<i class="fa-solid fa-tooth"></i>', "MERCADO": '<i class="fa-solid fa-cart-shopping"></i>', "RESTAURANTES": '<i class="fa-solid fa-utensils"></i>', "CARRO": '<i class="fa-solid fa-car"></i>', "PASSEIOS": '<i class="fa-solid fa-ticket"></i>', "FARMÁCIA": '<i class="fa-solid fa-pills"></i>', "FARMACIA": '<i class="fa-solid fa-pills"></i>', "TRANSPORTE": '<i class="fa-solid fa-bus"></i>', "MANUTENÇÃO": '<i class="fa-solid fa-screwdriver-wrench"></i>', "MÓVEIS": '<i class="fa-solid fa-couch"></i>', "VIAGEM": '<i class="fa-solid fa-plane"></i>', "CELULAR": '<i class="fa-solid fa-mobile-screen"></i>', "ALIMENTAÇÃO": '<i class="fa-solid fa-burger"></i>', "DOCS": '<i class="fa-solid fa-file-lines"></i>', "COMPRAS": '<i class="fa-solid fa-bag-shopping"></i>', "BELEZA": '<i class="fa-solid fa-scissors"></i>', "DENTISTA/MED": '<i class="fa-solid fa-user-doctor"></i>', "SEGURO": '<i class="fa-solid fa-shield-halved"></i>', "OUTROS": '<i class="fa-solid fa-tag"></i>'};
...
// Datas de calendário: usar componentes locais, nunca toISOString() (UTC),
// para não avançar um dia após o fim da tarde/noite no fuso -03.
const localDateKey=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
const mkey=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
const localLoad=()=>JSON.parse(localStorage.getItem(KEY)||"[]");
const load=()=>remoteRows;
const save=()=>{};
const addMonths=(date,n)=>{let d=new Date(date+"T12:00:00"),day=d.getDate();d.setDate(1);d.setMonth(d.getMonth()+n);d.setDate(Math.min(day,new Date(d.getFullYear(),d.getMonth()+1,0).getDate()));return localDateKey(d)};
...
function applyPrivacy(){
 ...
 if(btn){
   btn.innerHTML=valuesHidden?'<i class="fa-solid fa-lock"></i>':'<i class="fa-solid fa-eye"></i>';
...
function render(){
 ...
 const mIndex=current.getMonth(); const yNum=current.getFullYear(); $("#monthTitle").textContent=`${MONTH_LABELS[mIndex]} ${yNum}`;
 ...
 $("#categoryTotals").innerHTML=categoriesForTotals.length?categoriesForTotals.map(c=>{
   ...
   return `<div class="catrow"><div class="catname"><span class="caticon">${ICONS[c.category]||"📌"}</span><span>${c.category.toUpperCase()}<span class="catmeta">${[...c.groups].map(g=>g).join(" · ")}</span></span></div><div class="catvalue-wrap"><div class="catvalue">${money(c.total)}</div><div class="catavg">MÉDIA ${periodLabel()} · ${money(avg)}</div></div></div>`;
 ...
 if(shown.length){
   $("#transactions").innerHTML=shown.map(x=>{
     ...
     const icon=inc?'<i class="fa-solid fa-money-bill-wave"></i>':third?'<i class="fa-solid fa-handshake"></i>':(ICONS[x.category]||'<i class="fa-solid fa-tag"></i>');
     let meta="";
     if(inc){
       ...
       meta=`<span class="group-tag group-${x.group.toLowerCase()}">${x.group}</span> · ${x.category} · ${x.date.split("-").reverse().join("/")} · ${paymentText}${x.installmentLabel?" · "+x.installmentLabel:""}${x.recurringLabel?" · 🔁 "+x.recurringLabel:""}`;
     }
...
function updatePayment(){
 ...
 const installmentField=$("#installmentField");
 if(installmentField){
   if(isCard){
...
function updateEntryType(){
 ...
 if($("#paidByLabel")) $("#paidByLabel").classList.toggle("hidden",inc||third);
 ...
 if($("#recurringToggleLabel")) $("#recurringToggleLabel").classList.toggle("hidden",inc||third||($("#splitPayment").checked&&expense));
...
function openEdit(id){
 const x=loadNorm().find(t=>t.id===id);if(!x)return;editingId=id;$("#expenseForm").reset();{$("#expenseDialogTitle").innerHTML='<i class="fa-solid fa-pen"></i> LANÇAMENTO';}$("#editHint").classList.remove("hidden");$("#entryType").value=x.entryType||"EXPENSE";$("#amount").value=x.amount;$("#description").value=(x.description===x.category?"":x.description);$("#date").value=x.date;
...
$("#fab").onclick=()=>{editingId=null;$("#expenseForm").reset();{$("#expenseDialogTitle").innerHTML='<i class="fa-solid fa-plus"></i> LANÇAMENTO';}$("#editHint").classList.add("hidden");$("#entryType").value="EXPENSE";$("#splitPayment").checked=false;$("#splitAmountW").value="";$("#splitAmountC").value="";$("#date").value=localDateKey(new Date());$("#group").value="CASAL";$("#payment").value="CARD";$("#installments").value=1;let d=new Date();d.setMonth(d.getMonth()+11);$("#recurringUntil").value=mkey(d);updateCategories();updatePayment();updateRecurring();updateEntryType();applyPrivacy();applySystemTheme();updateSplitPayment();$("#expenseDialog").showModal()};
...
function exportCurrentMonth(){
 ...
 if(typeof XLSX==="undefined"){
   alert("NÃO FOI POSSÍVEL CARREGAR O EXPORTADOR XLSX. VERIFIQUE A CONEXÃO E TENTE NOVAMENTE.");
   return;
 }

 // ---------------- ABA 1: RESUMO ----------------
 const summary=[["GRUPO","CATEGORIA","WILLIAM","CAROL","TOTAL"]];
 let gw=0,gc=0;
 ...
 for(const group of ["CONTAS","ALICE","CASAL","WILLIAM","CAROL"]){
   for(const category of EXPORT_ORDER[group]){
     ...
     gw+=w;gc+=c;
     summary.push([group,category,w,c,w+c]);
   }
 }
 summary.push(["TOTAL GASTOS","",gw,gc,gw+gc]);
 summary.push([]);
 summary.push(["FATURAS","CARTÃO WILLIAM","CARTÃO CAROL","TOTAL"]);
 summary.push(["FATURAS",d.cards.W,d.cards.C,d.cards.W+d.cards.C]);
 summary.push([]);
 summary.push(["RECEITAS","CATEGORIA","WILLIAM","CAROL","TOTAL"]);
 ...
 for(const cat of incCats){
   ...
   iw+=w;ic+=c;
   summary.push(["RECEITAS",cat,w,c,w+c]);
 }
 summary.push(["TOTAL RECEITAS","",iw,ic,iw+ic]);
 summary.push([]);

 const tw=thirds.filter(x=>x.card==="W").reduce((s,x)=>s+Number(x.amount),0);
 const tc=thirds.filter(x=>x.card==="C").reduce((s,x)=>s+Number(x.amount),0);
 summary.push(["TERCEIROS","CARTÃO WILLIAM","CARTÃO CAROL","TOTAL"]);
 summary.push(["TERCEIROS",tw,tc,tw+tc]);

 // ---------------- ABA 2: LANÇAMENTOS ----------------
 const launchHeader=[
   "DATA","DESCRIÇÃO","TIPO","GRUPO","CATEGORIA","VALOR",
   "PAGO POR","RECEBIDO POR","PAGAMENTO","CARTÃO",
   "PARCELA","RECORRÊNCIA","WILLIAM PAGOU","CAROL PAGOU",
   "IMPACTA GASTOS","IMPACTA FATURA"
 ];
 ...
 const launchRows=sortedRows.map(x=>{
   const type=x.entryType==="INCOME"?"RECEITA":x.entryType==="THIRD_PARTY"?"TERCEIROS":"DESPESA";
   const split=hasSplit(x);
   const pagoPor=x.entryType==="EXPENSE"?(split?"WILLIAM + CAROL":(x.paidBy==="W"?"WILLIAM":x.paidBy==="C"?"CAROL":"")):"";
   const recebidoPor=x.entryType==="INCOME"?(x.receivedBy==="W"?"WILLIAM":x.receivedBy==="C"?"CAROL":""):"";

   let pagamento="",cartao="",williamPago=0,carolPago=0;
   if(x.entryType==="EXPENSE"){
     williamPago=paidW(x);carolPago=paidC(x);
     if(split){
       const wp=x.paymentSplit?.W,cp=x.paymentSplit?.C;
       pagamento=[
         wp?.amount?`WILLIAM: ${wp.payment==="CARD"?"CARTÃO":"CASH"}`:"",
         cp?.amount?`CAROL: ${cp.payment==="CARD"?"CARTÃO":"CASH"}`:""
       ].filter(Boolean).join(" | ");
       cartao=[
         wp?.payment==="CARD"?"CARTÃO WILLIAM":"",
         cp?.payment==="CARD"?"CARTÃO CAROL":""
       ].filter(Boolean).join(" | ");
     }else{
       pagamento=x.payment==="CARD"?"CARTÃO":x.payment==="$"?"CASH":"";
       cartao=x.payment==="CARD"?(x.card==="W"?"CARTÃO WILLIAM":x.card==="C"?"CARTÃO CAROL":""):"";
     }
   }else if(x.entryType==="THIRD_PARTY"){
     pagamento="CARTÃO";
     cartao=x.card==="W"?"CARTÃO WILLIAM":x.card==="C"?"CARTÃO CAROL":"";
   }
   ...
   return [
     ...
     type,
     x.group||"",
     x.category||(x.entryType==="INCOME"?(x.incomeCategory||""):""),
     Number(x.amount||0),
     pagoPor,
     recebidoPor,
     pagamento,
     ...
     williamPago,
     carolPago,
     x.entryType==="EXPENSE"?"SIM":"NÃO",
     (x.entryType==="THIRD_PARTY"||(x.entryType==="EXPENSE"&&(split
       ? (x.paymentSplit?.W?.payment==="CARD"||x.paymentSplit?.C?.payment==="CARD")
       : x.payment==="CARD")))?"SIM":"NÃO"
   ];
 });

 const wb=XLSX.utils.book_new();
 const wsSummary=XLSX.utils.aoa_to_sheet(summary);
 const wsLaunch=XLSX.utils.aoa_to_sheet([launchHeader,...launchRows]);

 // Larguras amigáveis.
 wsSummary["!cols"]=[
   {wch:18},{wch:24},{wch:16},{wch:16},{wch:16}
 ];
 wsLaunch["!cols"]=[
   {wch:12},{wch:30},{wch:13},{wch:14},{wch:20},{wch:14},
   {wch:18},{wch:18},{wch:30},{wch:22},{wch:12},{wch:16},
   {wch:16},{wch:16},{wch:14},{wch:14}
 ];

 // Filtro na aba de lançamentos.
 if(launchRows.length){
   wsLaunch["!autofilter"]={ref:`A1:P${launchRows.length+1}`};
 }

 // Formatação numérica real (não texto) para facilitar somas/filtros.
 const moneyFmt='#,##0.00';
 ...
 for(const col of ["F","M","N"]){
   for(let r=2;r<=launchRows.length+1;r++){
     const cell=wsLaunch[`${col}${r}`];
     if(cell&&cell.t==="n")cell.z=moneyFmt;
   }
 }

 XLSX.utils.book_append_sheet(wb,wsSummary,"RESUMO");
 XLSX.utils.book_append_sheet(wb,wsLaunch,"LANÇAMENTOS");
 XLSX.writeFile(wb,`contas_${key}.xlsx`);
}
...
$("#loginForm").onsubmit=async e=>{
 ...
 try{
   ...
   if(error){
     ...
     box.innerHTML="<strong>LOGIN NÃO REALIZADO</strong><br>Confira o e-mail e a senha e tente novamente.";
     box.classList.remove("hidden");
     return;
   }
   if(!data?.session){
     box.innerHTML="<strong>LOGIN NÃO REALIZADO</strong><br>Não foi possível iniciar a sessão. Tente novamente.";
     box.classList.remove("hidden");
   }
 }catch(err){
   console.error("Falha inesperada no login:",err);
   box.innerHTML="<strong>ERRO INESPERADO</strong><br>Tente novamente em instantes.";
   box.classList.remove("hidden");
 }
};
